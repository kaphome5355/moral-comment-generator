...
const handler: Handler = async (event) => {
  ...
  try {
    const { valueItem, keywords } = JSON.parse(event.body || "{}");

    if (!valueItem || !Array.isArray(keywords)) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "無効なリクエストデータです。" }),
      };
    }
...
•	教材の登場人物になりきって、同じような経験を思い出し、どのような気持ちだったかを発言し、自分事として考えていました。特に、「友だち」に関わる学習では「大丈夫？と声をかけてもらって嬉しかったから、自分も声をかけたい」と、具体的な姿を思い描き、今後に生かそうとしていました。
•	教材の登場人物になりきって、同じような経験を思い出し、どのような気持ちだったかを発言し、自分事として考えていました。特に、「友達」に関わる学習では、「本当の友達なら、話したら分かってくれるから、これからも、本当の気持ちを言える友達をたくさん作っていきたい」と、具体的な姿を思い描き、今後に生かそうとしていました。
•	教材の登場人物になりきって、同じような経験を思い出し、どのような気持ちだったかを発言し、自分事として考えていました。特に、「親切や思いやり」に関わる学習では「体育で失敗した時に、大丈夫だよと声をかけてもらって嬉しかったから、自分も声をかけたい」と、具体的な姿を思い描き、今後に生かそうとしていました。
...
•	教材の登場人物になりきって、自分の経験を思い出し、どのような気持ちだったかを考えていました。特に、相手の立場を考える学習では「けんかをして自分から謝ることは難しいけれど、勇気をもって謝っている人がいる」と発表し、自分もそういう人になりたいと、考えを新たにしました。
•	教材の登場人物になりきって、自分の経験を思い出し、どのような気持ちだったかを考えていました。特に、「友達」に関わる学習では、「けんかをしたら、悲しい気持ちになるからけんかはしたくない」と、具体的な姿を思い描き、今後に生かそうとしていました。
...
`;
    ...
    return {
      statusCode: 500,
      // クライアント側で一貫したエラーメッセージを生成するため、ここでは接頭辞をつけません。
